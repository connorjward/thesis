...
\begin{itemize}
  ...
  \item
    An \emph{axis component} is an object that also carries a \emph{label} as well as a \emph{size} (usually an integer).
\end{itemize}
...
They generalise \numpy{} N-dimensional arrays to ragged and multi-component settings and are capable of representing more structure PETSc DMPlex can using \ccode{PetscSections}.

As we will show in \cref{chapter:firedrake}, the sorts of complex data layouts that occur in continuum mechanics calculations are straightforward to describe using axis trees without losing important structural information and this facilitates the development of expressive interfaces.
