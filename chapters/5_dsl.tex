...
% NOTE: Doesn't really flow, and repeated as subsubsections below.
% \subsection{Code generation}
% \label{sec:pyop3_codegen}

\todo[inline]{explain inames and how domains are specified}
...
% also the name of the subsection!
...
\begin{itemize}
  ...
  \item
    The \pycode{loop} expression declares that one should loop over the 5 entries in axis $a$ and evaluate the local kernel (\pycode{kernel}), passing in indexed versions of \pycode{dat0} and \pycode{dat1}.
...
Like Fortran, local kernels in \pyop3 need to declare argument \emph{intents}.
...
% TODO: also the name of the subsection!
\subsubsection{Code generation}
...
\begin{itemize}
  ...
  \item
    \ccode{write} and \ccode{inc} nodes are handled in the same way as \cref{example:assign_codegen}.
...
Further, \pycode{Mats} do not use \numpy arrays internally to store data; instead they use PETSc matrices (also called \ccode{Mats}).
...
\begin{center}
  \includegraphics{mat_loop_expr.pdf}
\end{center}
Generating code from these nodes follows an identical process to that described in \cref{sec:pyop3_codegen} with one exception: the matrix packing operation `\ccode{write(t0, mat[(f(ix),:), (f(ix),:)])}'.
Since the data is stored in a PETSc \ccode{Mat} instead of a \numpy array, its values must be accessed using the PETSc C API, instead of directly addressing offsets in a memory buffer.

In order to read from and write to a PETSc \ccode{Mat}, the functions \ccode{MatGetValues()} and \ccode{MatSetValues()} must be used respectively.
...
\begin{cinline}
  MatGetValues(Mat mat,
               ...
               PetscScalar v[])
\end{cinline}
Here \ccode{mat} is the PETSc matrix object and \ccode{v[]} the dense array containing the read values.
...
Observe that the data type of \ccode{v[]} is \ccode{PetscScalar}; unlike other data structures in \pyop3, matrices are only able to store values with whatever floating point type PETSc was configured with.
...
\todo[inline]{Reference chapter 1 for inspector-executor}
...
Due to the work-in-progress nature of \pyop3 none of the listed optimisations have yet been implemented, but they are included in this thesis as examples of the potential that the abstraction has.
...
Consider, for example, a flat \pycode{Dat} indexed with the map composition $g(f(p))$ where $p$ is some outer loop and $f$ and $g$ are maps tabulated with arrays \pycode{map_f} and \pycode{map_g} respectively.
...
Expression compression is a similar technique to that done in~\cite{dasSlicingAnalysisIndirect1994}, and is an example of the inspector-executor approach to code generation (\cref{sec:introduction_software}).
...
This optimisation therefore provides a way to reproduce existing \pyop2 performance.
