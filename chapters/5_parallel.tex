...
As with Firedrake and PETSc, MPI is chosen as the sole parallel abstraction; hybrid models also using shared memory libraries like OpenMP (cite) are not used because the posited performance advantages are contentious \pcite{knepleyExascaleComputingThreads2015} and would increase the complexity of the code.

\section{Message passing with star forests}

Almost all message passing in \pyop3 is handled by star forests, specifically by PETSc star forests (\ccode{PetscSF}) \pcite{zhangPetscSFScalableCommunication2021}.
...
\pyop3 associates with each parallel data structure two attributes: \py{leaves_valid} and \py{pending_reduction}.
...
If the leaves of the forest were modified, \py{pending_reduction} stores the reduction operation that needs to be executed for the roots of the star forest to contain correct values.
As an example, were values to be incremented into the leaves\footnote{For this to be valid the leaves need to be zeroed beforehand.}, a \clang{SUM} reduction would be required for owned values to be synchronised.
...
\begin{itemize}
  \item If the array is being written to \py{op3.WRITE}, all prior writes may be discarded.
  \item If the array is being read from (\py{op3.READ}) and all values are already up-to-date, no exchange is necessary.
  \item If the array is being incremented into (\py{op3.INC}) multiple times in a row, no exchange is needed as the reductions commute.
\end{itemize}
