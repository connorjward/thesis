...
First, instead of constructing a multi-component \pycode{"mesh"} axis from the start, we instead construct a single component axis with the component representing all points in the mesh:
\begin{pyinline}
...
To further improve locality, the outer loop over cells is ordered with an reverse Cuthill-McKee numbering~\cite{cuthillReducingBandwidthSparse1969} (\pycode{cell_order} in \cref{alg:plex_renumbering}).
...
\pyop2 has no support for runtime transformations and so non-Lagrange elements are not supported on hexahedral meshes in Firedrake.
