...
Obtaining portable performance when solving PDEs is challenging because it requires high-level modifications to algorithms and data structures so as to best take advantage of the properties of the underlying hardware.
To hide the complexity inherent in making such changes, packages exist providing high-level \emph{abstractions} with which the necessary transformations become straightforward to express.
This leads to a separation of concerns whereby the package user can write their program using objects that are familiar to them, and they need not concern themselves with lower-level details.

It is difficult to develop high-level abstractions for mesh stencil calculations because unstructured mesh data structures are complex and map poorly to existing representations of structured data (e.g. N-dimensional arrays).
This means that existing abstractions for mesh stencils are limited in what they can express, either working for a range of different algorithms or a range of complex data structures, but not both.

This thesis introduces a novel suite of abstractions for describing unstructured mesh data.
Unlike existing approaches, the abstraction is fully capable of describing complex data layouts without losing essential structural information, allowing for an expressive, composable interface suitable for representing a wider range of algorithms.
...
\begin{itemize}
  ...
  \item The implementation of these abstractions as part of the new mesh stencil library \pyop3, which has been integrated into the finite element library Firedrake~\cite{FiredrakeUserManual}.
\end{itemize}
...
\begin{itemize}
  \item
    \Cref{chapter:background} describes some of the relevant background, motivating the need for mesh stencil computations and identifying shortcomings with existing approaches.
  \item
  ...
  \item
    \Cref{chapter:pyop3} presents \pyop3, a new mesh stencil package built upon these abstractions.
    Additional considerations to enable distributed memory parallelism in \pyop3 are then discussed in \cref{chapter:parallel}.
  \item \Cref{chapter:firedrake} describes how \pyop3 has been integrated into the finite element framework Firedrake, and \cref{chapter:demonstrator_applications} demonstrates this integration with a number of examples.
...
In addition to providing a zero-cost abstraction, by representing the problem at a high level, it is sometimes possible for the compiler to apply specialised optimisations that are difficult to apply by hand.
Whilst it is always the case that any automatically generated code could also have been written by hand, compilers frequently generate code with comparable or even superior performance to hand-written implementations (e.g.~\cite{ragan-kelleyHalideLanguageCompiler2013}).
...
Examples include hybridisation~\cite{arnoldMixedNonconformingFinite1985,boffi2013mixed} and additive-Schwartz methods~\cite{arnoldPreconditioningHdivApplications1997}.
...
\pyop2 is a reimplementation of the OP2 API in Python, using run-time code generation to compile the parallel loops instead of source-to-source translation.
...
Compared with \cref{fig:scott_vogelius_element_dof_layout_packed}, one can immediately see that the \pycode{"mesh"} layer of the structure is no longer present, illustrating that the relation between nodes and their owning mesh entities is lost.
