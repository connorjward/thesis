...
For mesh stencil calculations it is difficult to developing high-level abstractions because unstructured mesh data structures are complex and map poorly to existing representations of structured data (e.g. N-dimensional arrays).
...
\begin{itemize}
  ...
  \item The implementation a this new abstraction as part of the new mesh stencil library \pyop3, which has been integrated into the finite element library Firedrake~\cite{FiredrakeUserManual}.
\end{itemize}
...
In this chapter we introduce the relevant background necessary for understanding the relevance of mesh stencil calculations for solving PDEs and motivate the need for a new abstraction for mesh data.
...
At this scale, implementing high-level abstractions as function calls is not suitable because the additional overhead incurs a substantial performance penalty.
