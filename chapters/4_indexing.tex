...
\end{equation}
which, applying the substitutions from the target expressions, produces the substituted layout function
\begin{equation}
  \textnormal{offset}(i_c, i_d) = 6 i_c + i_d + 1,
\end{equation}
thus mapping multi-indices in the source axis tree to offsets in the original array as desired.
...
As a result of this non-constant arity, the size of the indexed array is no longer constant but instead depends on the value of the loop index.
