...
\begin{enumerate}
  \item
    A PETSc \ccode{Section} (\cref{sec:dmplex_data_layout}) is constructed for the axis component.
    Since \ccode{Sections} are analogous to the tabulated offset arrays described in \cref{sec:layout_alg_ragged}, the algorithm is very similar to \cref{alg:tabulate_offsets}.

  \item
    The point star forest and \ccode{Section} are composed by calling the function \ccode{PetscSFCreateSectionSF()} to yield a DoF-to-DoF mapping for the axis component.
...
% NOTE: This is badly formatted, improve along with PyOP2 one!
\begin{algorithm}
