...
\pycode{Mats}, by using PETSc matrices as their underlying storage mechanism, are parallel by default and interact little with \pyop3.
...
To avoid copying data between ghost-inclusive and ghost-free representations of an array, \pyop3 follows the example of \pyop2 and reorders the DoFs so that ghost entries are always stored after owned ones.
...
\begin{pyinline}
  [("owned", num_owned), ("constrained", num_constrained), ("ghost", num_ghost)]
\end{pyinline}

\subsubsection{Comparison with \pyop2}

Rather than storing data for multi-field problems in a contiguous array, \pyop2 stores each field in a separate sub-array.
...
\pyop3, by using regions to partition the ghost data to the end of the array regardless of the field structure, is able to avoid this copy, improving performance.
...
In order to make sure that the data held by each \pycode{Dat} is valid, two attributes are stored:
\begin{description}
...
The reason that these attributes are tracked by \pyop3 is that they enable communication to occur lazily.
...
Instead \pyop2 would perform leaf-to-root reductions eagerly, which was found to be a bottleneck in certain finite element simulations with multiple assembly loops executed in sequence.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \For{\textit{item} \textbf{in} \textit{iterset.root}} \Comment{Do the computations that \textbf{do} need \rootiter data but \textbf{not} need \leafiter data}
      \State \Call{Compute}{\textit{item}}
    \EndFor

    \State \Call{BroadcastRootsToLeavesEnd}{} \Comment{Finish updating \leafiter points}

    \For{\textit{item} \textbf{in} \textit{iterset.leaf}} \Comment{Do the computations that \textbf{do} need \leafiter data}
      \State \Call{Compute}{\textit{item}}
    \EndFor
  \end{algorithmic}
...
\pyop3 therefore addresses the stated limitation of \pyop2 (\cref{sec:pyop2_parallel}) where the iteration set partition is decided at array initialisation, leading to incorrect partitioning for different stencil sizes.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \Require \textit{arrays} \Comment{Arrays that are used in the computation}

    \State \textit{labels} $\gets$ \Call{AllocArray}{\textit{iteration set.size}} \Comment{Array to hold the labels, defaults to \coreiter}
    \For{\textit{point} \textbf{in} \textit{iteration set}}
      \For{\textit{array} \textbf{in} \textit{arrays}}
        \If {\textit{labels}[\textit{point}] = \leafiter}
          \State \textbf{continue}
        \EndIf

        \For{\textit{stencil point} \textbf{in} \Call{CollectStencil}{point}}
          \State \textit{sflabel} $\gets$ \Call{GetStarForestLabel}{\textit{stencil point}}
          \If {\textit{sflabel} = \leafiter}
            \State \textit{labels}[\textit{point}] $\gets$ \leafiter
            \State \textbf{break} \Comment{No point checking any more}
          \ElsIf {\textit{sflabel} = \rootiter}
            \State \textit{labels}[\textit{point}] $\gets$ \rootiter
          \Else
            \State \textbf{pass} \Comment{Default value is \coreiter}
          \EndIf
        \EndFor
      \EndFor
    \EndFor
