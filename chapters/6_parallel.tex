...
In order to represent distributed parallel data layouts using axis trees, axis components are passed an additional \pycode{StarForest} object (\cref{sec:dmplex_parallel}) that encodes the point-to-point equivalence between processes:
\begin{pyinline}
  star_forest = StarForest(...)
  parallel_component = AxisComponent(size, sf=star_forest)
  parallel_axis = Axis(parallel_component)
\end{pyinline}
\pyop3 \pycode{StarForests} are simply thin wrappers around PETSc ones.
...
Instead \pyop2 performs leaf-to-root reductions eagerly, which was found to be a bottleneck in certain finite element simulations with multiple assembly loops executed in sequence.
  ...
  \begin{algorithmic}[1]
    ...
    \For{\textit{point} \textbf{in} \textit{iteration set}}
      \For{\textit{array} \textbf{in} \textit{arrays}} \Comment{Check the stencil of each array}
        ...
        \For{\textit{stencil point} \textbf{in} \Call{CollectStencil}{\textit{point}}}
...
%This chapter stops suddenly, which makes it feel incomplete. I think what is missing is a concluding section which explains why what has been presented is a complete parallel implementation. I think this should include how matrices fit in and hence why they weren't discussed.
