...
Rather more well-known than \pyop2 or DMPlex, \numpy~\cite{harrisArrayProgrammingNumPy2020} is the de-facto package for array-based computation in Python.
Core to its functionality is the \emph{N-dimensional array}, or \pycode{ndarray}, which is an abstraction and interface for representing multi-dimensional arrays of data.
...
To represent multi-dimensional arrays, \numpy combines a memory buffer, containing the actual values, with additional \emph{metadata} used to interpret the buffer.
In particular, the metadata includes the \emph{shape} of the array, its data type, and \emph{stride} information.
One could, for example, encode a $64 \times 64$ image as an \pycode{ndarray} with a 4096 byte buffer of values, a shape of \pycode{(64, 64)}, \pycode{uint8} as the data type, and strides of \pycode{(64, 1)}.
...
Alternative representations for the same array are termed \emph{views}.
...
% TODO: don't discuss these any more... rephrase
...
% changes the stride information in the metadata!
